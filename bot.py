...
from tel_bot.config import TOKEN, ADMIN_CHAT_ID
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    terms_keyboard = ReplyKeyboardMarkup(
        keyboard=[["✅ مطالعه کرده و موافقت میکنم"]],
        resize_keyboard=True,
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("term_"):
        ...
        try:
            # چک کن ببین آیا اطلاعات قبلاً ذخیره شده
            existing_status = session.query(StudentStatus).filter_by(telegram_user_id=telegram_user_id).first()

            if existing_status:
                prev_student_code = existing_status.student_number or "نامشخص"
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"📌 شما قبلاً اطلاعات ثبت کرده‌اید\nکد دانشجویی: <b>{prev_student_code}</b>\nآیا این اطلاعات را تایید می کنید یا می‌خواهید تغییر دهید؟",
                    parse_mode="HTML",
                    reply_markup=confirm_keyboard
                )

            else:
                # اطلاعات جدید را ذخیره کن (اولین باره)
                status_data = {
                    "telegram_user_id": telegram_user_id,
                    "student_number": student_code
                }

                save_success = save_student_status(status_data)

                if save_success:
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text="✅ اطلاعات دانشجویی شما ثبت شد.",
    ...
    elif data == "select_unit" or data == "remove_unit":
